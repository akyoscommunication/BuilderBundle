    ...
    component." . $input->getArgument('name') . ":
      ...
      public: true";
        ...
        if (!file_exists($componentDir . $nameUCFirst)) {
            $output->writeln(['Création du dossier' . $componentDir . $nameUCFirst . '...']);
            if (!mkdir($concurrentDirectory = $componentDir . $nameUCFirst) && !is_dir($concurrentDirectory)) {
                throw new RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
            }
            $output->writeln(['Le dossier a bien été créé']);
        }
        /*Controller*/
        if (!file_exists($componentComponentDir . $nameUCFirst . 'ComponentController.php')) {
            $output->writeln(['Création du Controlleur ' . $componentComponentDir . $nameUCFirst . 'ComponentController.php ...']);
        ...
        if (!file_exists($componentComponentDir . $nameLCFirst . '_component.html.twig')) {
            $output->writeln(['Création de la vue Twig ' . $componentComponentDir . $nameLCFirst . '_component.html.twig ...']);
            file_put_contents($componentComponentDir . $nameLCFirst . '_component.html.twig', "{{ dump() }}");
            $output->writeln(['Le fichier a bien été créé']);
        }
        $output->writeln(['Edition du services.yaml']);
